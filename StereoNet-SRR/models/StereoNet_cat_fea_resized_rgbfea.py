...
class FeatureExtraction(nn.Module):
    ...
    def forward(self, rgb_img):
        feature_list = [rgb_img]
        for i in range(self.k):
            feature_list.append(self.downsample[i](rgb_img))
            rgb_img = feature_list[-1]
        output = feature_list[-1]
        for block in self.residual_blocks:
            output = block(output)
        return self.conv_alone(output), feature_list
...
class StereoNet(nn.Module):
    def __init__(self, k, r, maxdisp=192):
        ...
        for _ in range(4):
            self.filter.append(
                nn.Sequential(
                    convbn_3d(64, 64, kernel_size=3, stride=1, pad=1),
                    nn.LeakyReLU(negative_slope=0.2, inplace=True)))
        ...
        for _ in range(r - 1):
            self.edge_aware_refinements.append(EdgeAwareRefinement(33))
        self.edge_aware_refinements.append(EdgeAwareRefinement(4))

    def forward(self, left, right):
        disp = (self.maxdisp + 1) // pow(2, self.k)
        refimg_fea, zoomed_refimg_fea = self.feature_extraction(left)
        targetimg_fea, zoomed_targetimg_fea = self.feature_extraction(right)

        # matching
        cost = torch.FloatTensor(refimg_fea.size()[0],
                                 refimg_fea.size()[1] * 2, disp,
                                 refimg_fea.size()[2],
