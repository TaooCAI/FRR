...
def __absolute2relative(data):
    for i, item in enumerate(data):
        for j, path in enumerate(item):
            s_path = path.split('/')
            label = list(filter(lambda x: 'monkaa' in x.lower(
            ) or 'driving' in x.lower() or 'flying' in x.lower(), s_path))[0]
...
def dataloader(filepath, *, only_train_on_flyingthings3d=False):
    if only_train_on_flyingthings3d:
        store_path = \
            './flying3d_relative_pathlist_tuple_except_unused_files.pth'
    else:
        store_path = \
            './datasets_relative_pathlist_tuple_except_unused_files.pth'
    if os.path.exists(store_path):
        print('load from {}'.format(store_path))
