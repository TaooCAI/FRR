...
parser.add_argument(
    '--datapath',
    default='/media/jiaren/ImageNet/data_scene_flow_2015/training/',
    help='datapath')
parser.add_argument(
    '--epochs', type=int, default=300, help='number of epochs to train')
parser.add_argument(
    '--loadmodel', default='./trained/submission_model.tar', help='load model')
...
all_left_img, all_right_img, all_left_disp, \
    test_left_img, test_right_img, test_left_disp = ls.dataloader(
        args.datapath)
...
def train(imgL, imgR, disp_L):
    ...
    if args.model == 'stackhourglass':
        ...
        loss = 0.5 * F.smooth_l1_loss(output1[mask], disp_true[mask], size_average=True) \
            ...
            + F.smooth_l1_loss(output3[mask],
                               disp_true[mask], size_average=True)
    elif args.model == 'basic':
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (imgL, imgR, disp_L) in enumerate(TestImgLoader):
            ...
            print('Iter %d 3-px error in val = %.3f' % (batch_idx,
                                                        test_loss * 100))
